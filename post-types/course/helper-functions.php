	...
	function academist_lms_course_add_users_to_course( $id ) {
		...
		foreach ( $items as $item ) {
			...
			if ( $item['product_id'] != 0 && get_post_type( $product_id ) == 'course' ) {
				...
				$user_status_values = academist_lms_get_user_courses_status( $order->get_user_id());
				...
				if ( ! empty( $user_status_values ) ) {
					if ( ! array_key_exists( $product_id, $user_status_values ) ) {
						...
						academist_lms_set_user_courses_status( $user_status_values,  $order->get_user_id() );
					}
				} else {
					...
					academist_lms_set_user_courses_status( $user_status_new_values,  $order->get_user_id() );
				}
			}
		}
	}
...
if ( ! function_exists( 'academist_lms_set_user_courses_status' ) ) {
	function academist_lms_set_user_courses_status( $user_status_values , $user_id = '' ) {
...
if ( ! function_exists( 'academist_lms_get_user_courses_status' ) ) {
	function academist_lms_get_user_courses_status($user_id = '') {
		$user_id = isset( $user_id ) && ! empty( $user_id ) ? $user_id : get_current_user_id();
